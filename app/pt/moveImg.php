    ...
    foreach ($posts as &$post) {
        ...
        if (preg_match($search, $key)) {
            ...
            if ($info == "") {
                ...
                $url = explode("downloads_v2/", $value);

                //echo $cont++ . " - " . $post->ID . " - " . $info;
                echo "s3cmd cp s3://files.adventistas.org/downloads/" . $url[1] . " s3://files.adventistas.org/" . $value;
                echo "\n";
            } else {

                $url = explode("downloads_v2/", $info['key']);


                //echo $cont++ . " - " . $post->ID . " - " . $info['key'];
                echo "s3cmd cp s3://files.adventistas.org/downloads/" . $url[1] . " s3://files.adventistas.org/downloads_v2/" . $url[1];
                echo "\n";
            }
        }
    }
